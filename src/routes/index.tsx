...
const PersonalWebsite = () => {
  ...
  onMount(() => {
    ...
    const setScrollDistance = () => {
      setCurrentIndex(0);
      if (window.innerWidth < 640) scrollDistance = window.innerWidth - 32 + 48;
      else scrollDistance = 28 * 16;
    };
    setScrollDistance();
    window.addEventListener("resize", setScrollDistance);
    onCleanup(() => window.removeEventListener("resize", setScrollDistance));
  });
  createEffect(() => {
          ...
          <div class="flex gap-4 md:gap-6 flex-wrap">
            <For each={footprints}>
              {(item) => (
                ...
                >
                  <i class={`inline-block ${item.icon} w-6 h-6`} />
                ...
                <div class="flex md:gap-8 gap-6 items-center">
                  <div>
                    ...
                    <div class="text-gray-500 flex items-center justify-end text-sm whitespace-nowrap">
                      ...
                      <span class="inline-block text-sm">{post.category}</span>
                    </div>
                  </div>
                  <a
                    href={post.url}
